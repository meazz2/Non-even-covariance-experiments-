...
def total_nll_full_params(params, loc, y_list, s1, s2, u0):
    a11, a22, a12_e, a12_o, theta, rho = params
    if a11 <= 0.1 or a22 <= 0.1 or a12_e <= 0.1 or a12_o <= 0.1 or abs(rho) > 0.99:
        return 1e20
    C = build_cov_C2(loc, a11, a22, a12_e, a12_o, theta, rho, s1, s2, u0)
    try:
...
def run_single_iteration(iteration_id, loc, L_true, true_params, s1, s2, u0, replicates):
    ...
    res = minimize(
        ...
        bounds=[(0.5, 10), (0.5, 10), (0.5, 15), (0.5, 15), (0, np.pi*2), (-0.95, 0.95)],
        method="L-BFGS-B",
        options={'maxiter': 50}
    )
    if not res.success:
...
if __name__ == "__main__":
    ...
    df_results = pd.DataFrame(results, columns=['a11', 'a22', 'a12_e', 'a12_o', 'theta', 'rho'])
