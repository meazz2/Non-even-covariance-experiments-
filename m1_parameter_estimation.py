...
def C2_block(h, a11, a22, a12, nu11, nu22, nu12, rho, s1, s2, u0):
    ...
    C21 = -C12 #- I removed minus for symmetric 
...
def build_cov_C2(loc, a11, a22, a12, nu11, nu22, nu12, rho, s1, s2, u0):
    ...
    for i in range(N):
        for j in range(N):
            h = loc[j] - loc[i]
            block = C2_block(h, a11, a22, a12, nu11, nu22, nu12, rho, s1, s2, u0)
...
def run_single_iteration(iteration_id, loc, L_true, true_params, s1, s2, u0, replicates):
    ...
    res = minimize(
        ...
        bounds=[(0.5, 10), (0.5, 10), (0.5, 15), (0.4,30), (0.4,30), (0.4,30),(-0.95, 0.95)],
        method="L-BFGS-B",
        options={'maxiter': 150}
    )
    if not res.success:
...
if __name__ == "__main__":
    ...
    df_results = pd.DataFrame(results, columns=['a11', 'a22', 'a12', "nu11", "nu22", "nu12", 'rho'])
    df_results.to_csv("simulation_results_50_rows_m1.csv", index=False)
