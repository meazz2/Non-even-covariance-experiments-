...
def total_nll_full_params(params, loc, y_list, s1, s2, u0):
    a11, a22, a12_e, a12_o,  nu11, nu22, nu12_e, nu12_o, theta, rho = params
    if a11 <= 0.1 or a22 <= 0.1 or a12_e <= 0.1 or a12_o <= 0.1 or abs(rho) > 0.99:
        return 1e20
    C = build_cov_C2(loc, a11, a22, a12_e, a12_o,  nu11, nu22, nu12_e, nu12_o, theta, rho, s1, s2, u0)
    try:
...
def run_single_iteration(iteration_id, loc, L_true, true_params, s1, s2, u0, replicates):
    ...
    random_start = [p * np.random.uniform(0.9, 1.1) for p in true_params[:-2]]
    random_start.append(true_params[-2] + np.random.uniform(-0.1, 0.1))
    random_start.append(true_params[-1] * np.random.uniform(0.9, 1.1))
    
    res = minimize(
        total_nll_full_params,
        x0=random_start,
        args=(loc, y_sim, s1, s2, u0),
        bounds=[(0.5, 10), (0.5, 10), (0.5, 15), (0.5, 15), (0.4,30), (0.4,30), (0.4,30), (0.4,30), (0, np.pi*2), (-0.95, 0.95)],
        method="L-BFGS-B",
        options={'maxiter': 150}
    )
    if not res.success:
...
if __name__ == "__main__":
    ...
    df_results = pd.DataFrame(results, columns=['a11', 'a22', 'a12_e', 'a12_o',"nu11", "nu22", "nu12_e", "nu12_o", 'theta', 'rho'])
    df_results.to_csv("simulation_results_50_rows.csv", index=False)

    # Independent Plotting
    fig, axes = plt.subplots(1, 10, figsize=(22, 5))
